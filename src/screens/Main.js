...
const Main = ({navigation}) => {
  ...
  const [news,setNews]= useState(useContext(DataContext).data.results)

  const fetchData = async inpData => {
    try {
      const response = await axios.post('http://10.10.0.67:8000/summarize', {
        text: inpData,
      });
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.newsDisplayContainer}>
        ...
        {news.map((item, key) => {
          return <NewsContainer key={key} navigation={navigation} data={item} />;
        })}
      </View>
    </ScrollView>
  );
